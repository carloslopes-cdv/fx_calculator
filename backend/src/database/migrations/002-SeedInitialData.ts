...
export class SeedInitialData1710000000002 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const b1 = books.find(
      (b: { name: string }) => b.name === 'Corporate FX Portfolio',
    )?.id;
    const b2 = books.find(
      (b: { name: string }) => b.name === 'Energy & Commodities Desk',
    )?.id;
    const b3 = books.find(
      (b: { name: string }) => b.name === 'Emerging Markets Desk',
    )?.id;

    console.log('🌱 Semeando 9 Trades...');
    const trades = (await queryRunner.query(`
      INSERT INTO trades (book_id, side, currency_pair, volume, entry_rate, trade_date) VALUES
        -- Book 1: Corporate FX Portfolio (Status: HEALTHY ~ 88.5% Cobertura)
        ...
        -- Book 2: Energy & Commodities Desk (Status: WARNING ~ 62.5% Cobertura)
        ...
        -- Book 3: Emerging Markets Desk (Status: CRITICAL ~ 27.2% Cobertura)
    ...
    await queryRunner.query(`
      INSERT INTO hedges (trade_id, volume, entry_rate, hedge_date) VALUES
        ...
        -- Hedges do Book 2 (Média Cobertura)
        ...
        -- Hedges do Book 3 (Baixa Cobertura)
